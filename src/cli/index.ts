...
Usage:
  ...
  agent-eval triage <trace> --format <fmt>   Analyze a trace export for process failures
...
/**
 * `agent-eval triage <trace> --format <fmt>` - deterministic Tier-1 analysis over
 * a trace export. Post-hoc and offline: it reads trace facts and reports what
 * failed the process (timeouts, aborts, runaway spend, errors). It is a REPORT,
 * not a gate. The human reads it and decides the fix (code or prompt).
 */
async function runTriage(parsed: ParsedArgs): Promise<void> {
  const tracePath = parsed.paths[0];
  if (!tracePath) {
    console.error('Usage: agent-eval triage <trace> --format <otlp|langsmith|agentlens>');
    process.exit(1);
    return;
  }
  if (!parsed.format) {
  ...
  if (parsed.json) {
    // Pure JSON on stdout: the report, nothing else.
  ...
  // Triage is a report, not a gate: always exit 0. The human decides what to do
  // with the findings — the loop is closed by a human feeding fixes to the agent.
  process.exit(0);
}
