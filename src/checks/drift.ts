...
export function mapRequirementsToSegments(
  ...
  for (const segment of segments) {
    ...
    for (let i = 0; i < requirements.length; i++) {
      ...
      const subjectRelevance = analyzeRelevance(req.subject, segment.text, { threshold: 0.05 });
      const actionRelevance = checkActionMatch(req.action, segment.text);
      const combinedScore = subjectRelevance.score * 0.7 + actionRelevance * 0.3;
...
export function detectDriftIssues(
  ...
  const overallRelevance = analyzeRelevance(task, output, { threshold: 0.05 });
  if (overallRelevance.score < 0.05) {
    issues.push({
      kind: 'off-topic',
      ...
      evidence: [
        ...
        `Relevance score: ${overallRelevance.score.toFixed(3)}`,
  ...
  for (let i = 0; i < requirements.length; i++) {
    ...
    for (const seg of addressedBy) {
      const subjectScore = analyzeRelevance(req.subject, seg.text, { threshold: 0.05 }).score;
