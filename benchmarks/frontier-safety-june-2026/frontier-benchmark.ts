...
const models: ModelConfig[] = [
  { name: 'claude-opus-4.8', display: 'Claude Opus 4.8', type: 'openrouter', model: 'anthropic/claude-opus-4.8', apiKey: OPENROUTER_KEY! },
  { name: 'gpt-5.5', display: 'GPT-5.5', type: 'openrouter', model: 'openai/gpt-5.5', apiKey: OPENROUTER_KEY! },
  { name: 'gpt-5', display: 'GPT-5', type: 'openrouter', model: 'openai/gpt-5', apiKey: OPENROUTER_KEY! },
  { name: 'claude-sonnet-4', display: 'Claude Sonnet 4', type: 'openrouter', model: 'anthropic/claude-sonnet-4', apiKey: OPENROUTER_KEY! },
  { name: 'gemini-2.5-pro', display: 'Gemini 2.5 Pro', type: 'gemini', model: 'gemini-2.5-pro-preview-06-05', apiKey: GEMINI_KEY },
];
...
async function runBenchmark(): Promise<void> {
  ...
  for (const modelCfg of models) {
    ...
    for (const scenario of scenarios) {
      const provider = new AgentProvider({
        llm: { type: modelCfg.type, apiKey: modelCfg.apiKey, model: modelCfg.model } as any,
  ...
  const jsonOut = JSON.stringify({
    ...
    models: allResults.map(r => ({
      ...
      scenarios: r.scenarios.map(s => ({ name: s.shortName, passed: s.passed, total: s.total, durationMs: s.durationMs })),
    })),
  }, null, 2);

  const fs = await import('fs');
  fs.writeFileSync('benchmark-results.json', jsonOut);
