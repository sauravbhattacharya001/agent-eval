...
// module so this file stays focused on the loop, tool dispatch, and the backend
// HTTP calls. The public types are re-exported below so the surface at
// `./providers/agent.js` is unchanged; the wire types (ChatMessage,
// ChatCompletionResponse, OpenAICompatibleSpec) are imported for internal use only
// and deliberately NOT re-exported. See ./agent-types.ts.
import type {
  ToolDefinition,
  CapturedToolCall,
  AgentTurn,
