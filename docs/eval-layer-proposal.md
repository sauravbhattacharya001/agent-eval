...
| [#1302](https://github.com/anthropics/claude-code-action/issues/1302) | Action posts the repo's `CLAUDE.md` **verbatim** as the comment body instead of a structured review — *and the workflow still exits success* | **Coverage / relevance** — the output doesn't cover the prompt's topics and isn't about *this* PR; it's a generic document |
...
Four checks, all **deterministic** — Tier 1 (externally observable: the agent
can't forge a parse result or a missing turn) and Tier 2 (statistically
observable: the agent didn't write the baseline being compared against). **No
model-as-judge. No extra network calls. No added API cost. No flake.** They run
on bytes the action already wrote.
...
- **Keyword coverage** *(Tier 2, recall)* — does the output cover the *prompt's*
  topics, or is it a generic document that ignores the task? Targets #1302's
  "posted `CLAUDE.md` instead of a review" mode.
- **Relevance** *(Tier 2, precision)* — is the output *about THIS* PR/issue, or
  generic advice that merely name-drops the keywords? TF-IDF cosine against the
  prompt.

Any single failing check trips the gate. The semantics, thresholds, and the
rationale for why these four diverge in practice live in the
...
2. **Cost & determinism in CI.** A gate that adds an API call, a second source
   ...
   These four are pure functions of the run's bytes.
3. **The 80%.** The reported failures (#1368/#1361/#1302/#1308) are *all* in the
   deterministic band — stale, abandoned, empty, off-task. Shipping the 80% that
   Tier 1+2 covers solves the filed problems without the 20% that needs judgment.
...
- uses: anthropics/claude-code-action@v1
  with:
    ...
    eval_checks: "completeness,staleness,coverage,relevance"
```
...
1. **Prompt availability.** The execution file records the conversation, not the
   task framing (the action passes the prompt via a prompt file). Coverage +
...
- **`src/action/cca-execution.ts`** — total, dependency-free parser/extractor
  ...
  throws.
- **`src/action/ci-run.ts`** — `evaluateCiRun`, the four-check single-run gate.
